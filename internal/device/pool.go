...
type Pool struct {
	...
	// 流量采集：上一采样点的累计收/发字节基线（按设备 ID 记录）
	trafficPrevMu sync.Mutex
	trafficPrev   map[string]trafficSample
...
func NewPool(cfg *config.Config) *Pool {
	...
	p := &Pool{
		...
		trafficPrev:           make(map[string]trafficSample),
...
func (p *Pool) applyNetworkPreference(worker *Worker) error {
	...
	if worker.Config.NetworkEnabled {
		if worker.MBIMCore != nil {
			...
			// 修复：QMI 驻网成功后继续执行 StartNetwork()，
			// 而不是提前 return nil 跳过数据连接建立
		}
		if nc.IsConnected() {
...
func (p *Pool) startPoolBackgroundServicesOnce() {
	...
	p.startOnce.Do(func() {
		...
		go p.runTrafficAggregation()
		go p.runNetworkConfigDaemon()
