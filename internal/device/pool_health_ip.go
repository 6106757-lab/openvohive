...
func (p *Pool) refreshIPs(worker *Worker, checkPublic bool) {
	...
	go func() {
		...
		if checkPublic {
			...
			// 修复：公网 IP 探测失败时 fallback 到私有 IP
			// CGNAT 环境下 api.ipify.org 等可能无法访问，导致一直重试
			effectiveV4 := publicV4
			effectiveV6 := publicV6
			if effectiveV4 == "" && effectiveV6 == "" {
				// 公网探测全部失败，使用私有 IP 作为 fallback
				effectiveV4 = privateIP
				effectiveV6 = privateIPv6
				logger.Debug(fmt.Sprintf("[%s] 公网 IP 探测失败，fallback 到私有 IP", worker.ID),
					"private_v4", effectiveV4, "private_v6", effectiveV6)
			}

			if ipHealthy(effectiveV4, effectiveV6) {
				...
				if oldIP == "" && oldIPv6 == "" {
					logger.Info(fmt.Sprintf("[%s] 获取到公网 IP", worker.ID), "public_ip", effectiveV4, "public_ipv6", effectiveV6, "private_ip", privateIP, "private_ipv6", privateIPv6)
				} else if (effectiveV4 != "" && effectiveV4 != oldIP) || (effectiveV6 != "" && effectiveV6 != oldIPv6) {
					logger.Info(fmt.Sprintf("[%s] 检测到 IP 变更", worker.ID), "old_ip", oldIP, "new_ip", effectiveV4, "old_ipv6", oldIPv6, "new_ipv6", effectiveV6, "private_ip", privateIP, "private_ipv6", privateIPv6)

					if effectiveV4 != "" && oldIP != "" {
						if app := p.voWiFiHost().Instance(worker.ID); app != nil {
							logger.Info(fmt.Sprintf("[%s] 检测到内网抖动，正平滑触发底层 MOBIKE 漫游", worker.ID), "new_ip", effectiveV4)
							if err := app.TriggerMOBIKE(oldIP, effectiveV4); err != nil {
								logger.Warn(fmt.Sprintf("[%s] MOBIKE 漫游触发失败", worker.ID), "err", err)
							}
						}
					} else if effectiveV6 != "" && oldIPv6 != "" {
						if app := p.voWiFiHost().Instance(worker.ID); app != nil {
							logger.Info(fmt.Sprintf("[%s] 检测到 IPv6 内网抖动，正平滑触发底层 MOBIKE 漫游", worker.ID), "new_ipv6", effectiveV6)
							if err := app.TriggerMOBIKE(oldIPv6, effectiveV6); err != nil {
								logger.Warn(fmt.Sprintf("[%s] MOBIKE 漫游触发失败", worker.ID), "err", err)
							}
						}
					}
				...
				if imei := worker.getIMEI(); imei != "" {
					_ = db.UpdateDeviceIPsV6(imei, effectiveV4, effectiveV6, privateIP, privateIPv6)
				}
			} else {
