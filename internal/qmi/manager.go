...
func buildQMIManagerConfig(cfg config.DeviceConfig, device qmimanager.ModemDevice) qmimanager.Config {
	...
	return qmimanager.Config{
		...
		EnableIPv6:      enableV6,
		AutoReconnect:   true,
		// NoRoute 必须为 false：OpenWrt 下 netcfg 指向我们的 OpenWrtConfigurator，
		// 其 AddDefaultRoute 只把网关写入 pending，并不会直接写内核路由；
		// 真正的默认路由由 applyNetifdStatic 交给 netifd 以 metric=owWANMetric() 建立。
		// 若设为 true，库会跳过 AddDefaultRoute，我们将拿不到网关，
		// 生成的 static 接口就没有默认路由——反而更糟。
		NoRoute: false,
		NoDNS:           !netcfg.IsOpenWrt(), // 普通 Linux 不碰 DNS；OpenWrt 上交给 netifd 下发
...
// UpdateIPFamily 同步底层 QMI Manager 的 IPv4/IPv6 双栈开关。
// 上层在运行时切换 ip_version（如 v4 -> v4v6）后必须调用，否则 IPv6 数据呼叫不会发起。
func (m *Manager) UpdateIPFamily(enableV4, enableV6 bool) {
	if m.qmiMgr != nil {
