...
// SetIPFamily 动态更新 IP 双栈开关。Manager 的 EnableIPv4/EnableIPv6 在构建时从
// IPVersion 固化，上层在运行时切换 ip_version（如 v4 -> v4v6）后需要同步，否则 IPv6
// 数据呼叫永远不会被发起。该方法允许不重建 Manager 即可生效。
func (m *Manager) SetIPFamily(enableV4, enableV6 bool) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.cfg.EnableIPv4 = enableV4
	m.cfg.EnableIPv6 = enableV6
}
...
func (m *Manager) RotateIP() error {
	...
	// 1. Disconnect data call (V4 + V6) / 1. 断开数据呼叫 (双栈)
	...
	// 3.5 重建 IPv6 数据呼叫 / Re-establish IPv6 data call
	// 切换IP 必须重建双栈, 否则仅刷新 V4 而 V6 呼叫从未重建, FlushAddresses 又清掉了
	// IPv6 地址, 导致切换后 IPv6 永久丢失。此处仿照 doConnect 重新发起 V6 拨号。
	if m.cfg.EnableIPv6 && m.wdsV6 != nil {
		v6handle, e := m.wdsV6.StartNetworkInterface(ctx,
			m.cfg.APN, m.cfg.Username, m.cfg.Password,
			m.cfg.AuthType, qmi.IpFamilyV6)
		if e != nil {
			m.log.WithError(e).Warn("IPv6 重拨失败 (保留 IPv4)")
		} else {
			m.handleV6 = v6handle
		}
	}

	// 4. Reconfigure network (dual-stack) / 4. 重新配置网络 (双栈)
...
func (m *Manager) rotateViaRadioReset() error {
	...
	// 1. Disconnect current call (dual-stack) / 1. 断开当前呼叫 (双栈)
	if m.handleV4 != 0 && m.wds != nil {
...
registered:
	...
	// 5.5 重建 IPv6 数据呼叫 (切换IP 双栈一致性)
	if m.cfg.EnableIPv6 && m.wdsV6 != nil {
		if v6handle, e := m.wdsV6.StartNetworkInterface(ctx,
			m.cfg.APN, m.cfg.Username, m.cfg.Password,
			m.cfg.AuthType, qmi.IpFamilyV6); e != nil {
			m.log.WithError(e).Warn("radio-reset 后 IPv6 重拨失败 (保留 IPv4)")
		} else {
			m.handleV6 = v6handle
		}
	}
...
func (m *Manager) doConnect() error {
	...
	if m.cfg.EnableIPv6 && m.wdsV6 != nil {
		...
		if err != nil {
			m.log.WithError(err).Warn("IPv6 dial failed, will retry in background")
			// 启动后台 IPv6 重试，不阻塞 IPv4 连接
			go m.retryIPv6InBackground()
		} else {
			m.handleV6 = handle
...
// retryIPv6InBackground 在后台重试 IPv6 拨号，使用指数退避策略
func (m *Manager) retryIPv6InBackground() {
	delays := []time.Duration{5, 10, 20, 40, 80, 160}
	for i, delay := range delays {
		...
		if m.State() != StateConnected {
			m.log.Debug("IPv6 retry skipped: not connected")
			return
		}

		if m.wdsV6 == nil {
			m.log.Debug("IPv6 retry skipped: wdsV6 not available")
			return
		}

		// 检查当前 IPv6 是否已有地址（可能被其他流程重建了）
		m.mu.RLock()
		haveV6 := m.settings != nil && m.settings.IPv6Address != nil
		m.mu.RUnlock()
		if haveV6 {
			m.log.Debug("IPv6 already up, skip retry")
			return
		}

		m.log.Infof("Retrying IPv6 dial (attempt %d/%d)...", i+1, len(delays))
		ctx, cancel := m.opContext(m.cfg.Timeouts.Dial)
		handle, err := m.wdsV6.StartNetworkInterface(ctx,
			m.cfg.APN, m.cfg.Username, m.cfg.Password, m.cfg.AuthType, qmi.IpFamilyV6)
		cancel()
		if err != nil {
			m.log.WithError(err).Warnf("IPv6 retry %d failed", i+1)
			continue
		}

		m.handleV6 = handle
		m.log.Infof("IPv6 retry succeeded, handle=0x%08x", handle)

		// 重新配置 IPv6 地址到网卡
		if err := m.configureNetwork(); err != nil {
			m.log.WithError(err).Warn("IPv6 network reconfiguration failed")
		}
		return
	}
	m.log.Warn("IPv6 dial retries exhausted, giving up")
}
...
func (m *Manager) configureNetwork() error {
	...
	if m.wdsV6 != nil {
		...
		if err != nil {
			...
			m.settings.IPv6DelegatedPrefix = settingsV6.IPv6DelegatedPrefix
			m.settings.IPv6DelegatedPrefixLen = settingsV6.IPv6DelegatedPrefixLen
			if m.settings.MTU == 0 {
			...
			if settingsV6.IPv6Address != nil {
				m.log.Infof("Configuring IPv6: %s/%d (PD: %s/%d)",
					settingsV6.IPv6Address, settingsV6.IPv6Prefix,
					settingsV6.IPv6DelegatedPrefix, settingsV6.IPv6DelegatedPrefixLen)
				...
				// 委派前缀(PD)决定: 优先用 modem 下发的真正 PD(TLV 0x57)；
				// 若运营商只给单个 /64、未下发独立 PD，则把 wwan 自身 IPv6 地址所在的 /64
				// 当作 PD 写入 ip6prefix，使 odhcpd 仍能向 LAN 下发 IPv6
				// （实测中国移动等仅下发 /64，无独立委派前缀）。
				pd := settingsV6.IPv6DelegatedPrefix
				pdLen := settingsV6.IPv6DelegatedPrefixLen
				if pd == nil && settingsV6.IPv6Address != nil {
					pd = settingsV6.IPv6Address.Mask(net.CIDRMask(64, 128))
					pdLen = 64
				}
				if pd != nil && pdLen > 0 {
					m.settings.IPv6DelegatedPrefix = pd
					m.settings.IPv6DelegatedPrefixLen = pdLen
					if err := netcfg.SetIPv6DelegatedPrefix(ifname, pd, pdLen); err != nil {
						m.log.WithError(err).Error("Failed to set IPv6 delegated prefix")
					}
				}

				// 用户明确要求不填 IPv6 网关(ip6gw): 改用 on-link 默认路由 (default dev <iface>),
				// 否则错误的全局地址网关会导致 IPv6 不可达。
				if !m.cfg.NoRoute {
					m.log.Info("Adding on-link IPv6 default route (no ip6gw per operator requirement)")
