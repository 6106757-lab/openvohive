...
type NetworkConfigurator interface {
	...
	// SetIPv6DelegatedPrefix 将运营商委派的前缀(PD)写入接口(netifd 的 ip6prefix),
	// 使 odhcpd 能向 LAN 自动下发该前缀下的 IPv6 地址。
	// OpenWrt 平台会真正写入 ip6prefix; 其他平台为 no-op。
	SetIPv6DelegatedPrefix(ifname string, prefix net.IP, prefixLen int) error
