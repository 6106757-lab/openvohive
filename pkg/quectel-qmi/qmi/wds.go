...
const (
	...
	// TLVWDSIPv6DelegatedPrefix 是 IPv6 委派前缀(PD) TLV。
	// QMI WDS Get Runtime Settings 通过此 TLV 下发运营商委派的前缀(供 LAN 分发)。
...
type RuntimeSettings struct {
	...
	IPv6DNS1    net.IP
	IPv6DNS2    net.IP
	// IPv6DelegatedPrefix 是运营商委派的前缀(PD)。
	// 写入 netifd 接口的 ip6prefix 后, odhcpd 会自动向 LAN 分发该前缀下的 IPv6 地址。
...
func (w *WDSService) GetRuntimeSettings(ctx context.Context, ipFamily uint8) (*RuntimeSettings, error) {
	...
	// Parse IPv6 Delegated Prefix (PD) / 解析IPv6委派前缀(PD)
	// 不同模块对该 TLV 的字节序存在差异, 这里做容错解析:
	//   约定A(常见, 与 TLV 0x25 同序): 16字节前缀 + 1字节前缀长度
	//   约定B(部分模块):              1字节前缀长度 + 16字节前缀
	// PD 写入 ip6prefix 后, odhcpd 会自动向 LAN 下发 IPv6 地址。
	if tlv := FindTLV(resp.TLVs, TLVWDSIPv6DelegatedPrefix); tlv != nil {
		switch {
		case len(tlv.Value) >= 17:
			prefix := make(net.IP, 16)
			copy(prefix, tlv.Value[0:16])
			prefixLen := int(tlv.Value[16])
			if prefixLen < 1 || prefixLen > 128 {
				// 回退到约定B: 长度在前
				prefixLen = int(tlv.Value[0])
				copy(prefix, tlv.Value[1:17])
			}
			if prefixLen >= 1 && prefixLen <= 128 {
				settings.IPv6DelegatedPrefix = prefix
				settings.IPv6DelegatedPrefixLen = prefixLen
			}
		case len(tlv.Value) == 16:
			// 仅前缀, 默认 /64
			prefix := make(net.IP, 16)
			copy(prefix, tlv.Value)
			settings.IPv6DelegatedPrefix = prefix
			settings.IPv6DelegatedPrefixLen = 64
		}
	}
	...
	// [DEBUG] dump TLV 类型与 PD(0x57) 原始字节, 仅当 VOHIVE_QMI_DEBUG=1。
	// 用于确认 modem 是否下发委派前缀(0x57)及其字节序(prefix-first / len-first)。
	if os.Getenv("VOHIVE_QMI_DEBUG") == "1" {
		var b strings.Builder
		for _, t := range resp.TLVs {
			fmt.Fprintf(&b, "0x%02x ", t.Type)
			if t.Type == TLVWDSIPv6DelegatedPrefix {
				log.Printf("[qmi-debug] IPv6DelegatedPrefix raw len=%d bytes=%x", len(t.Value), t.Value)
			}
		}
		log.Printf("[qmi-debug] GetRuntimeSettings (family=%d) TLV types: %s | IPv6Gateway=%s IPv6Address=%s",
			ipFamily, strings.TrimSpace(b.String()), settings.IPv6Gateway, settings.IPv6Address)
	}
